...
The resulting pattern is stronger than the earlier generic stress audit. On every held-out row in Table~\ref{tab:simus_stress_frontier_headline}, the strongest detector-family stack outperforms the strongest public comparator. Under cardiac-like pulsation, the winning detector-family chain is always a fully whitened specialist, paired with RPCA on the reference and hard rows and with adaptive-global SVD on the moderate row. Under shortened ensembles, the winning chain remains detector-family dominated throughout: the fully whitened specialist is strongest at 64 and 48 frames, while the simpler matched-subspace default regains the lead at the hardest 32-frame setting. The main scientific use of this table is therefore not to replace the primary translational benchmark, but to show that once the nuisance is made more clinically realistic, the detector family does not merely avoid collapse; it keeps a substantial nuisance-control advantage over the strongest public stacks that were evaluated here.

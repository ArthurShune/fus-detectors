...
To isolate the role of the STAP core from baseline prefiltering, we run a small STAP-only ablation on the Gammex flow phantom by applying STAP directly to raw (registered) CFM IQ without the SVD band-pass baseline. We also provide a small PRF override sensitivity check on the along view to assess brittleness to the frozen PRF assumption required by Twinkling metadata limitations. The main text cites the TPR@$10^{-3}$ endpoints from this audit as a bounded fairness sanity check; the full figure below shows the paired first-20-frame comparison and PRF-override panel.

\ifSTAPSUPPLEMENTONLY
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/paper/twinkling_gammex_ablation_prf_sensitivity.pdf}
\caption{Gammex flow phantom: (left) STAP-only ablation comparing the standard baseline SVD band-pass + STAP pipeline to STAP-only on raw IQ (first 20 cine frames; both views). (right) PRF override sensitivity on the along view (first 20 cine frames). Points show TPR at a fixed FPR target of $10^{-3}$ with 95\% frame-bootstrap CIs under the structural tube masks. This is a low-cost sanity audit of the frozen PRF/band discretization, not a tuned performance sweep.}

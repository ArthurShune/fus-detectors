...
\noindent The supplementary material is organized as supporting evidence for the main paper, with structurally labeled and real-IQ audits first, optional adaptive/KA diagnostics next, and archival fairness or repository-style material later.
\begin{itemize}[nosep]
  \item \textbf{Supporting structural audits:} full ROC shapes for labeled regimes, cross-window calibration, whitened-head stability, relaxed-tail sanity checks, open-skull parameter sensitivity, and the Gammex STAP-only / PRF sensitivity check (Figure~\ref{fig:brain_kwave_roc_curves}, Figure~\ref{fig:twinkling_gammex_structural_roc_curves}, Figure~\ref{fig:brain_openskull_profile_sensitivity}, Figure~\ref{fig:twinkling_gammex_ablation_prf_sensitivity}).
  \item \textbf{Adaptive and KA supporting audits:} finalized threshold sensitivity, conditional-execution leakage, and artifact-heavy calculi hygiene (Tables~\ref{tab:adaptive_tau_sensitivity}, \ref{tab:ka_cflow_sensitivity}, and \ref{tab:condstap_leakage_ablation}).
...
\subsection*{Adaptive and KA Supporting Audits}
...
The main paper is structural-first, so the held-out functional same-cube audit is kept here. It fixes the residual stream and asks a narrower question: under a common functional readout, does the fully whitened detector improve activation-versus-nuisance separation relative to conventional Doppler-style score heads on the same held-out data? The answer is directionally positive but secondary. In the strict held-out audit, the fully whitened detector is positive in 5 of 6 residual families in both settings, with median held-out selection-score gains of +0.065 [0.032, 0.091] for the intra-operative parenchymal regime and +0.111 [0.032, 0.142] for the mobile regime. The corresponding median gains in activation-versus-nuisance AUC are +0.026 [0.014, 0.052] and +0.059 [0.011, 0.066]. Those effects are not large enough to carry the paper by themselves, but they remain consistent across the prespecified same-cube comparison.

...
\paragraph{Clinically grounded nuisance stress audit (reduced-grid SIMUS).}
One remaining fairness concern is that the held-out structural checkpoint may still be too clean: perfect or near-perfect summary metrics can invite the criticism that the task is trivial rather than that the detector is meaningfully better. To address that directly, we run a supplementary reduced-grid audit on the same mobile scene family using the open SIMUS/PyMUST forward model but perturb two nuisance axes that are explicitly grounded in the fUS literature: stronger residual bulk-motion fields and stronger nuisance reflectivity \cite{DemeneClutterSVD2015,BarangerAdaptiveSVD2018,Soloukey2020FUSAwake,MobileFUS2025}. This audit is intentionally bounded. It is not a replacement for the full-resolution held-out benchmark in the main paper, and it is not introduced as a second place where the proposed head must dominate every comparator. Its purpose is narrower: show that when clinically motivated nuisance is increased, the task stops looking perfect.

\input{reports/simus_clinical_stress_provenance_table.tex}
\input{reports/simus_clinical_stress_mobile_table.tex}

The resulting pattern is the one a skeptical reviewer should want to see. Once nuisance is increased, the same-residual MC--SVD comparison no longer sits in the trivial 1.000/0.000 regime. Under the reduced-grid mobile audit, nuisance-separation AUCs move into the 0.48--0.56 range and nuisance leakage at matched target sensitivity rises into the 0.42--0.59 range. The fixed band-limited matched-subspace head remains competitive and is modestly better than PD at the reference point and on the hardest bulk-motion row, but it is not uniformly dominant under every stronger perturbation. We therefore interpret this table as difficulty calibration rather than as a new headline efficacy result: it demonstrates that the stressor families advocated by reviewers do make the SIMUS task visibly harder, while leaving the main translational claim anchored to the prespecified full-resolution held-out benchmark in the main text.

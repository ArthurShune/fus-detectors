...
\noindent The supplementary material is organized as reader-facing support for the main paper, with structurally labeled and real-IQ audits first, optional adaptive/KA diagnostics next, and archival fairness or repository-style material later.
\begin{itemize}[nosep]
  ...
  \item \textbf{Supplementary simulation and held-out benchmark details:} detailed labeled brain tables, corrected SIMUS/PyMUST fairness checkpoints, and the use-case-motivated synthetic checkpoint tables (Tables~\ref{tab:simus_frozen_benchmark_v1p2}, \ref{tab:simus_symmetric_pipeline_v1p2}, \ref{tab:simus_v2_structural_latency}, and \ref{tab:simus_v2_functional_latency}).
...
To distinguish end-to-end stack selection from detector-family behavior more sharply, we also run a held-out same-residual detector-family ablation on the use-case-motivated synthetic structural cases using a common MC--SVD residual and the current translational STAP profile. This is intentionally not a replacement for Table~\ref{tab:simus_v2_structural_latency}: that table compares frozen end-to-end stacks, whereas Table~\ref{tab:simus_detector_family_ablation} changes only the downstream score head. Under this fixed residual family, the unwhitened band-limited matched-subspace ratio is currently the best-performing fixed nuisance-control head in both the mobile and intra-operative parenchymal settings, while whitened total power performs poorly. The implication is narrower than ``whitening failed'': the main translational frontier still favors STAP in full-stack comparisons on other residual families, but the same-residual MC--SVD audit shows that covariance whitening is not yet the dominant ingredient on every accepted residual stream.

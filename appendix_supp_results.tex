...
The completed 10-block / 20-window audit uses the fully whitened power-style specialist because that is the strongest short-window head on this structural occupancy target. On short 64-frame windows, the specialist reaches AUC $0.538$ [0.529, 0.548], compared with 0.476 [0.467, 0.486] for PD and 0.475 [0.465, 0.485] for Kasai. At matched 70\% vessel-core recall, the same specialist lowers perivascular-shell false positives from 0.723 [0.711, 0.736] for PD and 0.725 [0.712, 0.738] for Kasai to 0.635 [0.615, 0.654]. The effect is bounded and modest in absolute magnitude, but it is directionally consistent. This detector choice is also endpoint-specific: the structural vascular occupancy target on sparse-bubble IQ favors broader energy recovery under local whitening, whereas the held-out SIMUS nuisance-control benchmark favors the spectrally selective matched-subspace ratio. The direction remains positive under short 32-frame windows (AUC $0.534$ [0.520, 0.547] versus 0.477 [0.467, 0.488] for PD) and under injected \texttt{brainlike} motion at 1 px amplitude (AUC $0.523$ [0.516, 0.530] versus 0.472 [0.466, 0.477] for PD). A direct latency check on the same frozen 64-frame no-registration configuration yields 41.6~ms steady-state total on an RTX~4080 SUPER for a 64~ms acquisition budget. This is still a bounded structural-label audit rather than independent multimodal validation, and it should be interpreted as a structural vascular-fidelity result rather than as a functional-activation or hemodynamic-recovery benchmark.
...
\caption{ROC curves for the bounded localization-derived structural audit on ULM Zenodo 7883227 (10 evaluation blocks, 20 short windows). The vessel-core and perivascular-shell labels come from the published PALA localization rendering after registration back to the Zenodo IQ grid; the short-window detector scores are evaluated on the identical MC--SVD residual cube. On this endpoint, the fully whitened power-style specialist is stronger than PD and Kasai.}

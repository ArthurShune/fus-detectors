...
\item Compact baseline-family summary for the labeled Brain-* pilots: Table~\ref{tab:brain_baseline_summary}.
\end{itemize}
...
\endgroup
\caption{Brain-* cross-window threshold calibration audit for the pre-KA STAP score. ``Within-window'' matches the main reporting protocol (calibrate $\tau_\alpha$ on a window's negatives and evaluate on that same window). ``Cross-window'' calibrates $\tau_\alpha$ on one window and evaluates on a disjoint window ($n=20$ ordered window pairs). Reported values are median (IQR) over windows/pairs.}
\label{tab:brain_crosswindow_calibration}
\end{table}
\else
\noindent In the paper build, the cross-window threshold-transfer audit is promoted to the main Results section (Table~\ref{tab:brain_crosswindow_calibration}) to emphasize deployable calibration (train on one window, evaluate on another) and to quantify realized FPR drift.
\fi
...
\subsection{Brain-* strict-tail operating points}
\label{app:brain-kwave-vnext-table}
The detailed Brain-* low-FPR operating-point table is placed here so that the main text can foreground the accepted clinically anchored \emph{SIMUS-ClinUtility-v2} checkpoint. The Brain-* pilots remain important as a labeled strict-tail stress test and as the basis for the detector-component ablations that follow.
...
STAP (pre-KA) & local & robust covariance $\rightarrow$ whitening $\rightarrow$ flow-subspace ratio & fixed profile & matched-subspace & main Methods \\
\hline
\end{tabular}
}
\endgroup
\caption{Compact family summary for the labeled Brain-* pilot baselines. Baseline rows share the same PD flow-evidence score on their respective residual cubes; STAP differs by applying tile-local covariance whitening and a matched-subspace flow-band score on top of the MC--SVD residual.}

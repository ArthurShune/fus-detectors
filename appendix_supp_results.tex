...
When KA is enabled, it is shrink-only with protected-set invariance; labeled ROC reporting uses the pre-KA STAP score.
\end{minipage}}
\end{center}
...
The main Brain-* tables calibrate the operating threshold $\tau_\alpha$ on the negative set of the \emph{same} 64-frame window being evaluated. To quantify how sensitive these strict low-FPR operating points are to window-to-window distribution shift (and to avoid over-interpreting quantized tail points), we perform a simple cross-window audit: for each regime and FPR target $\alpha$, we calibrate $\tau_\alpha$ on one window's negatives and apply it to the other windows. Table~\ref{tab:brain_crosswindow_calibration} reports within-window TPR (as in the main text) alongside cross-window TPR and the realized cross-window FPR.
...
\begin{tabular}{P{3.0cm} c P{4.3cm} P{4.3cm} P{4.4cm}}
\hline
Regime & $\alpha$ & within-window TPR med (IQR) & cross-window TPR med (IQR) & cross-window realized FPR med (IQR) \\
\hline
Brain-OpenSkull & $10^{-4}$ & 0.0223 (0.0031,0.3922) & 0.0254 (0.0018,0.2226) & 1.59e-04 (0,1.25e-03) \\
Brain-OpenSkull & $3\times 10^{-4}$ & 0.2107 (0.0077,0.4610) & 0.2253 (0.0045,0.3409) & 3.19e-04 (0,1.82e-03) \\
Brain-OpenSkull & $10^{-3}$ & 0.2834 (0.1496,0.5590) & 0.3026 (0.1158,0.4226) & 1.15e-03 (7.47e-05,3.38e-03) \\
Brain-AliasContract & $10^{-4}$ & 0.3499 (0.2649,0.3756) & 0.3303 (0.2534,0.3998) & 1.10e-04 (0,5.28e-04) \\
Brain-AliasContract & $3\times 10^{-4}$ & 0.4033 (0.3283,0.4560) & 0.4016 (0.3188,0.4569) & 2.99e-04 (5.48e-05,9.21e-04) \\
Brain-AliasContract & $10^{-3}$ & 0.5063 (0.4383,0.5113) & 0.4927 (0.4096,0.5416) & 9.76e-04 (3.54e-04,2.18e-03) \\
Brain-SkullOR & $10^{-4}$ & 0.3791 (0.2622,0.3987) & 0.3491 (0.2624,0.4213) & 1.20e-04 (0,7.37e-04) \\
Brain-SkullOR & $3\times 10^{-4}$ & 0.4460 (0.3230,0.4648) & 0.4010 (0.3242,0.4741) & 3.19e-04 (2.99e-05,1.39e-03) \\
Brain-SkullOR & $10^{-3}$ & 0.5102 (0.4364,0.5456) & 0.5136 (0.4131,0.5377) & 9.86e-04 (1.74e-04,2.72e-03) \\
\hline
\end{tabular}
\endgroup
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/paper/ulm7883227_motion_sweep_ULM_blocks1_3_0000_0128_elastic_e975.pdf}
...
\caption{Mac{\'e} PD-only prior summary panel over all deduplicated scan/plane instances. Top: prior regions and the most common (region, reason) pairs (runtime, label-free). Bottom-left: coverage vs strength summaries for shrink-only penalization ($p_{\mathrm{shrink}}$ vs $\mathrm{IQR}(\log w)$ on penalized tiles), with the invariance threshold indicated. Bottom-right: offline atlas-stratified Pf-peak fractions (VIS/SC/LGd vs CP/CA/DG tiles) colored by $\Delta\log(E_a/E_f)$ (descriptive only; not a KA input). Reproducibility details are provided in \SuppOrApp{app:repro}.}
...
Accordingly, we interpret Shin results as evidence of (i) deterministic real-IQ ingestion and band-energy summaries, (ii) KA prior auditability and invariants (shrink-only; exact protected-set invariance), and (iii) self-consistency under controlled residual-motion corruption, while leaving ROC uplift claims to labeled regimes (Brain-* simulations) and to future raw-IQ datasets with task/landmark structure.

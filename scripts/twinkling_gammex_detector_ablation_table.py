...
`scripts/twinkling_eval_structural.py` for:
  - the default matched-subspace detector (`msd_ratio`)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        help="Along-view default matched-subspace summary JSON.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        help="Across-view default matched-subspace summary JSON.",
    )
    ap.add_argument(
...
def _render_table(
    ...
    method_display = {
        "Baseline (power Doppler)": "Power Doppler",
        "Baseline (Kasai lag-1 power)": "Kasai",
        "Detector ablation (whitened power)": "Whitened power",
        "Detector ablation (unwhitened ratio)": "Unwhitened ratio",
        "Fully whitened matched-subspace detector": "Fully whitened detector",
    }

    best_keys: set[tuple[str, str, float]] = set()
    for view in ("along", "across"):
        for fpr in fprs:
            tprs = [
                cell.tpr
                for method in method_order
                if (cell := cells.get((view, method, float(fpr)))) is not None and cell.tpr is not None
            ]
            if not tprs:
                continue
            best_tpr = max(tprs)
            for method in method_order:
                cell = cells.get((view, method, float(fpr)))
                if cell is not None and cell.tpr is not None and abs(cell.tpr - best_tpr) <= 1e-12:
                    best_keys.add((view, method, float(fpr)))

    def _cell(view: str, method: str, fpr: float) -> str:
        cell = cells.get((view, method, float(fpr)))
        text = _fmt_num(None if cell is None else cell.tpr)
        if (view, method, float(fpr)) in best_keys and text != "n/a":
            return f"\\textbf{{{text}}}"
        return text
    ...
    for idx, (view_key, view_label) in enumerate((
        ("along", "Along view (linear17 @ PRF 2500)"),
        ("across", "Across view (linear17 @ PRF 2500)"),
    )):
        if idx > 0:
            lines.append("\\medskip")
        lines.append(f"\\textbf{{{view_label}}}")
        lines.append("\\par\\smallskip")
        lines.append("\\begin{tabular}{p{3.2cm}cccc}")
        lines.append("\\hline")
        lines.append(
            "Method / score & "
        ...
        for method in method_order:
            ...
            for fpr in fprs:
                row.append(_cell(view_key, method, fpr))
    ...
    lines.append(
        ...
        "pooled lumen pixels. Point estimates are shown here for readability; full 95\\% frame-bootstrap CIs ($n=2000$ resamples over cine frames) are reported in the Supplementary Material. Pooled background counts are "
        ...
        f"($fpr_{{\\min}}\\approx {_fmt_sci_tex(fpr_min_across, decimals=1)}$). Best TPR in each column is bolded. "

...
Outputs:
  ...
  - LaTeX tabular fragment for the STAP rows used in Appendix B.2/Table 12.
"""
...
def main() -> None:
    ...
    tex_lines.append("\\begin{tabular}{P{3.0cm} c P{4.3cm} P{4.3cm} P{4.4cm}}")
    tex_lines.append("\\hline")
    tex_lines.append(
        "Regime & $\\alpha$ & within-window TPR med (IQR) & cross-window TPR med (IQR) & cross-window realized FPR med (IQR) \\\\"
    )
    tex_lines.append("\\hline")
    for regime_name in regimes_in_order:
        ...
        for alpha in alphas:
            ...
            cross_fpr = cross["fpr"]
            tex_lines.append(
                f"{regime_name} & {_fmt_alpha_tex(alpha)} & {_fmt_tpr_cell(within_tpr)} & {_fmt_tpr_cell(cross_tpr)} & {_fmt_fpr_cell(cross_fpr)} \\\\"
            )
    tex_lines.append("\\hline")

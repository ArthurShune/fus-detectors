...
In the PD-mode experiments in this paper we report operating points using the exported right-tail PD score map $S_{\mathrm{PD}}$ (stored as \texttt{score\_pd\_*.npy}): choose $\tau$ so that $\mathbb{P}\{S_{\mathrm{PD}}\ge\tau\mid \mathrm{bg}\}=\alpha$ and evaluate $\mathbb{P}\{S_{\mathrm{PD}}\ge\tau\mid \mathrm{flow}\}$. For compatibility, acceptance bundles may also include \texttt{pd\_*.npy}, which we denote $\mathrm{PD}^{\downarrow}$ and relate deterministically by $S_{\mathrm{PD}}=-\mathrm{PD}^{\downarrow}$.
...
\]
We also use a flow-coverage proxy $c_f(i)\in[0,1]$ (fraction of tile pixels in the flow mask), a depth proxy $d(i)\in[0,1]$ when available, and a baseline detector score $S_{\mathrm{base}}(i)$ on which KA operates (defined as a right-tail score so that higher means ``more flow-like''; in PD-mode this is $S_{\mathrm{base}}=S_{\mathrm{PD}}=\texttt{score\_pd}$).
...
\]
with $w_{\min}\in(0,1]$ so the corresponding scale $\gamma(i)=1/w(i)$ is finite, and set $w(i)=1$ on non-gated and protected tiles. In Doppler/IQ PD-mode bundles we export the per-pixel scale $\gamma(i)=1/w(i)\ge 1$ (averaged over overlapping tiles) and apply it on the union of gated tiles as
\[
\mathrm{PD}^{\downarrow}_{\mathrm{post}}(i)=\gamma(i)\,\mathrm{PD}^{\downarrow}_{\mathrm{pre}}(i),
\]
which inflates the legacy PD-mode map $\mathrm{PD}^{\downarrow}$ on gated regions. Since $S_{\mathrm{PD}}=-\mathrm{PD}^{\downarrow}$, this is equivalent to $S_{\mathrm{post}}=w\,S_{\mathrm{pre}}$ and therefore is non-increasing in score space. For PD-only detectors such as PD-z that take both signs, we use the hard-veto/soft-shrink forms in Appendix~\ref{app:pdonly-dashboard} so that shrink-only is literal for signed scores.

\subsubsection{Sentinels, reason codes, and auditability}
The contract disables KA (C0) when any of the following hold: insufficient background proxy support, flat risk telemetry (e.g.\ $\mathrm{IQR}(m_{\mathrm{risk}}\mid\mathcal{T}_{\mathrm{bg}})$ too small), guard domination that is not actionable, coverage outside bounds (gate inert or global), invariance (gated $w$ nearly constant), or monotonicity failure on near-protected tiles. When KA is active the contract records coverage and strength telemetry (e.g.\ $p_{\mathrm{shrink}}=\mathbb{P}\{g=1\mid \mathcal{T}_{\mathrm{cand}}\}$ and quantiles of $w$) along with a short reason string (e.g.\ \texttt{alias\_metric\_flat}, \texttt{guard\_dominates\_not\_actionable}, \texttt{coverage\_out\_of\_bounds}, \texttt{monotonicity\_guard\_failed}). This makes KA behavior reviewable and supports the intended posture: C0 is a success state, C1 is a conservative safety state, and C2 is a research-only state used to gate uplift claims.
...
\begin{itemize}[nosep]
  ...
  \item \emph{Behavior in skull/OR--like regimes.} In skull/OR realism regimes such as Brain--SkullOR, alias power deliberately leaks into $H_1$ and R--2 fails by construction. In these regimes KA is expected to be off or ROC--neutral; the primary detector is the fixed STAP PD profile, with MC--SVD/\allowbreak RPCA/\allowbreak HOSVD as reference baselines. This is a safety feature, not a failure of the method: whenever the contract conditions are not met, the system falls back to STAP alone rather than forcing KA to act.
\end{itemize}

...
A plain-language glossary for the appendix labels appears first in Appendix~\ref{app:appendix-guide}, and the main-text method overview is shown in Figure~\ref{fig:stap_pipeline}. This appendix then states the formal preconditions under which the optional shrink-only penalty layer is allowed to modify scores; if any precondition fails, the layer is disabled ($w\equiv 1$).
...
When reporting strict low-FPR operating points where $\tau$ is reselected after regularization to satisfy $\mathbb{P}\{S_{\mathrm{stap,post}}\ge\tau\mid H_0\}=\alpha$, any TPR change reflects background-tail compression despite scores never increasing.

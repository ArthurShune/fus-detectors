...
lam_used = float(info.get("lambda_used", 0.0))
ratio_no = ratio_map(Rt + 0.02 * torch.eye(Lt, dtype=Rt.dtype), S, Cf)
for alpha in [0.0, 0.25, 0.5, 0.75, 1.0]:
    mix = alpha * R_lam + (1 - alpha) * (Rt + lam_used * torch.eye(Lt, dtype=Rt.dtype))
    rq = float(torch.quantile(ratio_map(mix, S, Cf), 0.99).item())
    print(f"alpha {alpha:.2f} -> q99 {rq:.2f}")

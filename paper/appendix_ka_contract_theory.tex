\section{Shrink-Only Regularizer (Modality--Agnostic)}
\label{app:ka-contract-theory}

In this part we describe regime conditions and operator-level sufficient structure for a deterministic shrink-only score-space transform layered on top of a baseline slow-time detector. The analysis is formulated in terms of abstract band projectors $(\Po,\Pf,\Pg,\Pa)$, alias features, and score distributions, and does not depend on any particular imaging modality. Only in later sections do we instantiate these conditions for functional ultrasound (fUS) brain imaging with concrete band choices, acquisition parameters, and baselines.

\subsection*{How to read this appendix}
A plain-language glossary for the appendix labels appears first in Appendix~\ref{app:appendix-guide}, and the main-text method overview is shown in \ifSUPPLEMENTONLY the main paper's method-overview figure\else Figure~\ref{fig:stap_pipeline}\fi. This appendix then states the formal preconditions under which the optional shrink-only regularizer is allowed to modify scores; if any precondition fails, the layer is disabled ($w\equiv 1$).

\subsection{Shrink-only score mapping used in the main manuscript}
The main paper keeps the optional shrink-only regularizer to a brief summary because the primary efficacy claims do not depend on it. For completeness, we collect the core manuscript-level definitions here. The regularizer is driven by deterministic side-information features computed from the same tile-local slow-time vector $z_i$ and Doppler-band projectors $(\Po,\Pf,\Pg,\Pa)$ used by the detector:
\[
  E_f(i)=z_i\Hh \Pf z_i,\qquad
  E_g(i)=z_i\Hh \Pg z_i,\qquad
  E_a(i)=z_i\Hh \Pa z_i,
\]
with total in-band energy $E_{\mathrm{tot}}(i)=z_i\Hh z_i+\epsilon$. The main risk statistic is the alias-to-flow ratio
\[
  m_{\mathrm{alias}}(i)=\log\frac{E_a(i)+\epsilon}{E_f(i)+\epsilon},
\]
and we also track the guard-band fraction
\[
  r_g(i)=\frac{E_g(i)}{E_{\mathrm{tot}}(i)}.
\]
A deterministic flow-support proxy $c_f(i)\in[0,1]$ defines conservative background-like and flow-like proxy sets
\[
  \mathcal{T}_{\mathrm{bg}}:=\{i:\,c_f(i)\le c_{\mathrm{bg}}\},\qquad
  \mathcal{T}_{\mathrm{flow}}:=\{i:\,c_f(i)\ge c_{\mathrm{flow}}\}.
\]
These features are used only to decide whether shrinkage is warranted and where it may be applied; they are not used as class labels for ROC reporting.

The score-space mapping itself is bounded and monotone:
\[
  S_{\mathrm{det,post}}(i)=w(i)\,S_{\mathrm{det,pre}}(i),\qquad w(i)\in(0,1].
\]
The full construction of $w(i)$, including protected sets, regime logic, deterministic sentinels, and the reciprocal scale-map convention used in some exported audit files, is given by the appendix sections below.

\begin{lemma}[Shrink-only monotonicity]
\label{lem:shrink_only_monotonicity}
Let $w(i)\in(0,1]$ and define $S_{\mathrm{det,post}}(i)=w(i)\,S_{\mathrm{det,pre}}(i)$. For any threshold $\tau$,
\[
  \{S_{\mathrm{det,post}}\ge\tau\}\subseteq \{S_{\mathrm{det,pre}}\ge\tau\}.
\]
Consequently, on any subset $A$ (e.g.\ $H_0$), the tail probability cannot increase at fixed $\tau$:
\[
  \mathbb{P}\{S_{\mathrm{det,post}}\ge\tau\mid A\}\le \mathbb{P}\{S_{\mathrm{det,pre}}\ge\tau\mid A\}.
\]
\end{lemma}
\begin{proof}
If $S_{\mathrm{det,post}}(i)\ge\tau$, then $S_{\mathrm{det,pre}}(i)=S_{\mathrm{det,post}}(i)/w(i)\ge\tau$ since $w(i)\le 1$.
\end{proof}

When reporting strict low-FPR operating points where $\tau$ is reselected after regularization to satisfy $\mathbb{P}\{S_{\mathrm{det,post}}\ge\tau\mid H_0\}=\alpha$, any TPR change reflects background-tail compression despite scores never increasing.

\subsection{Regime Preconditions (Data--Level, Non--Negotiable)}
\label{sec:regime}

The most important empirical lesson is: even if the shrink-only operator $Q$ and band design $(\Pf,\Pa)$ are perfect in an abstract sense, \emph{uplift requires the data themselves to be in a compatible regime}. This section codifies that.
...
For the shrink-only regularizer to be meaningfully guided by alias evidence, we require \emph{class--differential separation} of $m_{\text{alias}}$:
\begin{equation}
\label{eq:R2}
...
If \eqref{eq:R2} fails (alias score almost identical on $H_0$ and $H_1$), the alias-localization rule (Appendix~\ref{app:gating}) has no reliable signal and regularizer uplift cannot be expected.
...
If the baseline already achieves near--maximal pAUC at the FPR of interest, even a perfect shrink-only transform cannot provide significant gains; in this regime the regularizer can serve as a safety mechanism, but not as a source of measurable uplift.

\subsection{Operator--Level Sufficient Structure (Non--Negotiable Core)}
\label{sec:operator}

This section describes conditions on $Q$ alone under which the shrink-only regularizer behaves like a band--wise scalar rescaling of scores, up to small perturbations.
...
Interpretation: $Q$ preserves band subspaces up to small mixing; it does not rotate energy from Pf into Pa or vice versa in any substantial way. This rules out the failure mode where the regularizer accidentally feeds alias into the flow numerator.

\paragraph{Runtime label--free check.}
Let $\wh Q := R_\beta^{-1/2}\wh R^{1/2}$ be the realized shrink-only transform. Estimate
\[
...
The shrink-only regularizer reshapes covariances only within bands. We use either a commuting band-scalar transform with target gains $(s_f,s_a,s_o)$ or a trace-normalized blend $R_\beta$ with small diagonal lift; both enforce bandwise structure (no off-block priors). Localization restricts the regularizer to alias-dominated, low-coverage tiles using alias scores, guard ratios, optional flow coherence/coverage checks, baseline score windows, depth/PD/registration cues, and is validated via coverage rates $p_-,p_+$. Full localization definitions are provided in Appendix~\ref{app:gating} (PD-only instantiation in Appendix~\ref{app:pdonly-dashboard}).
...
Let $S^-_{\text{base}}$ be the baseline negative score distribution, with CDF $F_-$. Let $t_\alpha$ be the $\alpha$--quantile: $F_-(t_\alpha) = 1-\alpha$. After shrink-only regularization with gating, the negative CDF becomes a mixture:
\[
...
Let $p_+$ be the positive gating rate and assume gated positives suffer at most a multiplicative drop $\nu := s_f^2/s_a^2\cdot(1+O(\delta_\bullet+\varepsilon_{\text{mix}}))$. For any threshold $t'_\alpha$ satisfying $F'_-(t'_\alpha)=1-\alpha$, the TPR under regularization satisfies
\[
\mathrm{TPR}_{\text{reg}}(t'_\alpha) \ge \mathrm{TPR}_{\text{base}}(t_\alpha) - \eta\,\Phi(\nu),
\]
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  \item Regularizer operator: $\wh\varepsilon_{\text{mix}}\le\varepsilon_{\text{mix}}$ (OC--1); $\wh s_a/\wh s_f\ge\Lambda$ and $\wh\delta_\bullet\le 0.1$ (OC--2).
  ...
  \item Invariance sentinel: if $\wh s_a/\wh s_f\in[1-\epsilon,1+\epsilon]$ with $\epsilon\le 0.02$, the regularizer is effectively a scalar; disable it (see Section~\ref{sec:regime-sentinel}).
\end{enumerate}

\subsection{Shrink-only regularizer: score-space penalty}
\label{app:ka-contract-v2-full}

To keep the shrink-only regularizer conservative under heterogeneity, we implement a deterministic, feature-conditioned prior that acts in score space as a shrink-only penalty. Concretely, the post-regularizer score is defined as
\[
S_{\mathrm{post}}(i)=w(i)\,S_{\mathrm{pre}}(i),\qquad w(i)\in(0,1],
\]
equivalently $T_{\mathrm{post}}=T_{\mathrm{pre}}+\log w$ when working with log-likelihood scores (MAP log-odds decomposition). By construction, $w\equiv 1$ (no regularizer effect) is an explicit, expected outcome whenever the feature statistics are uninformative or fail integrity checks.
...
\begin{itemize}[nosep]
  \item \textbf{$\mathcal{R}_0$ (uninformative).} The prior is uninformative and $w\equiv 1$; output equals the baseline detector (matched-subspace detector on IQ; PD-only detector on PD-only). Feature statistics are still recorded.
...
\]
We also use a flow-coverage proxy $c_f(i)\in[0,1]$ (fraction of tile pixels in the flow mask), a depth proxy $d(i)\in[0,1]$ when available, and a baseline detector score $S_{\mathrm{base}}(i)$ on which the regularizer operates (defined in a flow-evidence convention so that higher means ``more flow-like''; in PD-mode this is $S_{\mathrm{base}}=S_{\mathrm{PD}}=\texttt{score\_pd}$).
...
The shrink-only regularizer must not disturb high-confidence flow tiles. We define a protected set
\[
...
\caption{Shrink-only regularizer summary panel on a representative sweep. Left: regime counts. Middle: top (regime, reason) labels. Right: summary of shrink coverage and strength on runs where $w\not\equiv 1$.}
...
Experience shows that even when OC/BG/CB/G conditions are roughly satisfied, there can exist regimes where the shrink-only regularizer is either inert or harmful (e.g.\ risk feature flat, localization never triggers or triggers everywhere, baseline ROC saturated). This section formalizes when the regularizer should be \emph{explicitly disabled}.
...
\begin{itemize}[nosep]
  \item \textbf{RS--1a: Band occupancy.} If R--1 fails (e.g.\ most flow peaks near DC instead of Pf, or alias peaks rarely in Pa), the band design is not physically realized; do not deploy the regularizer.
  \item \textbf{RS--1b: Alias separation.} If R--2 fails (alias scores $m_{\text{alias}}$ show negligible difference between classes), alias localization has no reliable signal; do not deploy the regularizer.
  ...
  \item \textbf{RS--1d: Baseline saturation.} If $\text{pAUC}_\text{base}$ in the target FPR range is already at or near maximum and the score ratio distribution under regularization has median close to 1 with tiny spread, you are in a ROC--invariant regime; the regularizer can be left off.
\end{itemize}

\subsection{Safety--1: Score Monotonicity Guard (Runtime)}

Even when regime preconditions and localization coverage hold, we require that the regularizer does not produce grossly pathological score transformations on \emph{penalized flow tiles}. Let $S_{\text{base}},S_{\text{reg}}$ denote baseline and regularized scores on the chosen metric (BR or GLRT).

For the set of penalized flow tiles, define
\[
r_i := \frac{S_{\text{reg}}(i)}{\max(S_{\text{base}}(i),\varepsilon)},\quad
u_i := \log r_i,
\]
...
\]
and $\tau_f$ the Kendall $\tau$ rank correlation between $S_{\text{reg}}$ and $S_{\text{base}}$ on these pixels.

We require:
\begin{itemize}[nosep]
  \item \textbf{Sample size.} Apply the guard only if the number of gated flow pixels exceeds a threshold $N_{\min}$ (e.g.\ $N_{\min}=200$). If not, mark the regularizer as ``too sparse to evaluate'' but do not disable it solely on this basis.
  \item \textbf{Flow monotonicity.} The regularizer is marked unsafe if either
  \begin{align*}
    &\mathrm{IQR}_f > \log 10\quad\text{(heterogeneity }>\times10\text{)},\quad\text{or}\\
    &\mathrm{median}_f > \log 10\quad\text{and}\quad |\tau_f| < 0.1,
  \end{align*}
  i.e.\ if the regularizer systematically boosts flow scores by more than an order of magnitude while destroying rank consistency, or applies extremely heterogeneous multiplicative effects.
\end{itemize}

If Safety--1 fails, the regularizer is disabled at runtime for that configuration and the system falls back to the unregularized matched-subspace detector.

\subsection{What the Regime Does Not Promise (Updated)}

The shrink-only regularizer may \emph{fail to provide uplift} or may be disabled by the regime sentinel / safety guard in the following cases:
...
\begin{itemize}[nosep]
  ...
  \item If R--2 alias separation fails, i.e.\ $m_{\text{alias}}$ shows no class--differential behavior, alias localization cannot preferentially capture alias--dominated negatives; the regularizer is out of regime.
  \item If localization is not class--differential (offline $p_+\approx p_-$) or $p_-$ is either tiny or nearly one, the regularizer is either inert or global; SL--2 no longer applies.
  \item If baseline ROC is already saturated in the target FPR range, even an ideal shrink-only transform cannot move the operating point significantly.
  \item If Safety--1 indicates that the regularizer acts in a grossly non--monotone way on penalized flow tiles (e.g.\ large heterogeneous boosts), it is disabled for that configuration.
\end{itemize}

In all these regimes, MC--SVD or matched-subspace-only scoring should be preferred; the shrink-only regularizer can be viewed as an optional add-on rather than a source of guaranteed uplift.

\subsection{Safety and Regime Sentinel Summary}

For deployed use it is helpful to summarize, in plain language, when the shrink-only regularizer is allowed to act and when it is automatically turned off.

\begin{itemize}[nosep]
  \item \emph{When the regularizer is allowed to act.} The regularizer is enabled only in regimes that satisfy the structural preconditions (R--1/R--2/R--3), have useful but not global localization coverage (roughly $0.2\le p_-\le 0.5$ with $p_+\ll p_-$), and pass the invariance and Safety--1 checks. In this case it behaves as a controlled, band--aware add-on on top of the matched-subspace detector: it is permitted to shrink scores on alias--dominated negatives but must remain monotone and mild on flow tiles.
  \item \emph{When the regularizer is turned off.} The regularizer defaults to $\mathcal{R}_0$ by design when band occupancy or alias separation fail (R--1/R--2), when localization becomes inert or non--differential, when the baseline ROC is already saturated in the target low--FPR band (R--3), or when Safety--1 detects non--monotone, highly heterogeneous score changes on penalized flow tiles. The invariance sentinel also forces $\mathcal{R}_0$ when the realized gains satisfy $\wh s_a/\wh s_f\approx 1$ so that the regularizer is effectively a scalar rescaling.
  \item \emph{Behavior in skull/OR--like regimes.} In skull/OR realism regimes such as the structured-clutter simulation pilot (internal profile ID \texttt{Brain-SkullOR}), alias power deliberately leaks into $H_1$ and R--2 fails by construction. In these regimes the regularizer is expected to be off or ROC--neutral; the primary detector is the fixed matched-subspace profile, with MC--SVD/\allowbreak RPCA/\allowbreak HOSVD as reference baselines. This is a safety feature, not a failure of the method: whenever the regime preconditions are not met, the system falls back to matched-subspace-only scoring rather than forcing the regularizer to act.
\end{itemize}

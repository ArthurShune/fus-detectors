...
\begin{itemize}[nosep]
  ...
  \item \textbf{Mac{\'e} whole-brain fUS (hemodynamic variant).} On mid-visual slices (scan1/2 planes 3--4), Pf is dominant in $\approx 0.5$ of H$_1$ tiles versus $\ll 0.25$ in H$_0$, and median $\log(E_{\mathrm{Pa}}/E_{\mathrm{Pf}})$ is lower in H$_1$ by $\approx 0.5$--0.9. An alias-based gate at TPR $\approx 0.5$ prunes null-tile detections for PD-z (e.g., 40/7 $\rightarrow$ 27/0 H$_1$/H$_0$ hits), illustrating the KA sentinel in hemodynamic space.
...
On mid-visual slices (scan1/2 planes 3--4), Pf is dominant in $\approx 0.5$ of H$_1$ tiles versus $\ll 0.25$ (plane 3) and $\ll 0.1$ (plane 4) in H$_0$; median $\log(E_{\mathrm{Pa}}/E_{\mathrm{Pf}})$ is lower in H$_1$ by $\approx 0.5$--0.9. An alias-based gate at TPR $\approx 0.5$ prunes null-tile PD-z detections with modest hit loss: on plane 3, PD-z yields 40 H$_1$/7 H$_0$ detections, alias-gated PD-z yields 27/0; on plane 4, 37/0 $\rightarrow$ 28/0. Pf-fraction provides a small diagnostic pAUC bump on plane 4 (0.000676 vs PD-z 0.000628) but is neutral elsewhere. Figures illustrating Pf peak fraction, alias distributions, and gated detections are provided for these planes.
\begin{figure}[h]
\centering
